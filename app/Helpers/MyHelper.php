...
if(!function_exists('getPrice')){
    function getPrice($product = null){
        ...
        $result['html'] .= '<div class="price uk-flex uk-flex-middle mt10">';
            $result['html'] .= '<div class="price-sale">'.(($result['priceSale'] > 0) ? convert_price($result['priceSale'], true) : convert_price($result['price'], true) ).'<span class="currency ml5">coin</span></div>';
            if($result['priceSale'] > 0){
                $result['html'] .= '<div class="price-old uk-flex uk-flex-middle">'.convert_price($result['price'], true).'coin <div class="percent"><div class="percent-value">-'.$result['percent'].'%</div></div></div>';
...
if(!function_exists('getVariantPrice')){
    function getVariantPrice($variant, $variantPromotion){
        ...
        $result['html'] .= '<div class="price-sale">'.(($result['priceSale'] > 0) ? convert_price($result['priceSale'], true) : convert_price($result['price'], true) ).'coin</div>';
        if($result['priceSale'] !== $result['price']){
            $result['html'] .= '<div class="price-old">'.convert_price($result['price'], true).'coin <div class="percent"><div class="percent-value">-'.$result['percent'].'%</div></div></div>';
        }
        return $result;
    }
}
...
if(!function_exists('renderDiscountInformation')){
    function renderDiscountInformation($promotion = null){
        if($promotion->method === 'product_and_quantity'){
            ...
            $discountType = ($promotion->discountInformation['info']['discountType'] == 'percent') ? '%' : 'coin';
...
if(!function_exists('renderDiscountVoucher')){
    function renderDiscountVoucher($voucher = null){
        ...
        $discount_type = ($voucher->discount_type == 'PERCENTAGE') ? '%' : 'coin';
