...
class ProductController extends FrontendController
{
    ...
    public function index($id, $request)
    {
        ...
        if ($customer) {
            $paidTransaction = Transaction::where('customer_id', $customer->id)
    ...
    public function success($transactionCode){
        ...
        if(auth('customer')->user()->id !== $transaction->customer_id){
            return redirect()->route('home.index')->with('error', 'Bạn không có quyền xem thông tin này');
        }
        $product = $this->productRepository->getProductById($transaction->product_id, 1);

        $system = $this->system;
        $seo = [
            'meta_title' => 'Thanh toán thành công cho transaction: '.$transactionCode,
            'meta_keyword' => '',
            'meta_description' => '',
            'meta_image' => '',
            'canonical' => route('account.success', $transactionCode),
        ];
        $template = 'frontend.product.product.success';
        $config = [];
        return view($template, compact(
            'config',
            'seo',
